...
def _preprare_data(imgs: ImageType, model: BaseSegmentor):
    ...
    cfg = model.cfg
    for item in cfg.test_pipeline:
        if item['type'] in ['LoadAnnotations', 'LoadMatAnnotations']:
            cfg.test_pipeline.remove(item)
    ...
    for img in imgs:
        ...
        data['inputs'].append(data_['inputs'][None])
        data['data_samples'].append(data_['data_samples'])

    # concat image tensor
    data['inputs'] = torch.cat(data['inputs'])
